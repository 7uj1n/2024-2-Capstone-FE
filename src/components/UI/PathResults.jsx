                    ...
                    >
                        <Card.Header>
                            ...
                            <div className="route-feedback">
                                <span className="like">👍 {route.positive}</span>
                                <span className="dislike">👎 {route.negative}</span>
                                <Button className="comment-btn" variant="outline-dark" size="sm" onClick={() => handleCommentClick(route, index)}>의견 달기</Button>
                            </div>
                        </Card.Header>
                        <Card.Body>
                            <Card.Text>평균 소요 시간: {formatDuration(route.totalTime)}</Card.Text>
                            <ListGroup variant="flush">
                                {route.type === 'bus' && route.segments.map((segment, idx) => (
                                    <ListGroup.Item key={idx}>
                                        <img
                                            src={getMarkerImage(route.type)}
                                            alt={route.type}
                                            style={{ width: '40px', marginRight: '10px' }}
                                        />
                                        <div>
                                            <div>{segment.bus_id}</div>
                                            <div className="station-direction">{segment.board_station} → {segment.alight_station}</div>
                                            <div className="pathStation-duration">소요 시간: {formatDuration(segment.duration)}</div>
                                        </div>
                                    </ListGroup.Item>
                                ))}
                                {route.type === 'car' && (
                                    <ListGroup.Item>
                                        ...
                                        <div>
                                            <div>자동차</div>
                                            <div className="pathStation-duration">소요 시간: {formatDuration(route.totalTime)}</div>
                                        </div>
                                    </ListGroup.Item>
                                )}
                            </ListGroup>
